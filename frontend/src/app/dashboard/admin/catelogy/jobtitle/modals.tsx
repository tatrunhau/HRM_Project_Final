...
export default function JobTitleModals({
        ...
        <DialogContent className="sm:max-w-lg">
            ...
            <div className="flex items-center gap-2">
                ...
                <div>
                    <DialogTitle className="text-xl font-bold text-slate-800">
                        {isEditing ? 'Cập Nhật Chức Danh' : 'Thêm Mới Chức Danh'}
                    </DialogTitle>
                    <DialogDescription>Quản lý danh mục các chức danh (vị trí công việc cụ thể).</DialogDescription>
                </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Mã chức danh <span className="text-red-500">*</span></label>
                <Input 
                    value={formData.jobtitlecode || ''} 
                    onChange={(e) => handleInputChange('jobtitlecode', e.target.value)} 
                    placeholder="VD: GD, NVKT..." 
                    disabled={isEditing} 
            ...
            {/* Hàng 2: Tên */}
            <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Tên chức danh <span className="text-red-500">*</span></label>
                <Input 
                    value={formData.name || ''} 
                    onChange={(e) => handleInputChange('name', e.target.value)} 
                    placeholder="VD: Giám đốc điều hành, Nhân viên kỹ thuật" 
                />
            </div>

             {/* Hàng 3: Mô tả */}
            <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Mô tả / Ghi chú</label>
                <textarea 
                    className="flex min-h-[100px] w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"
